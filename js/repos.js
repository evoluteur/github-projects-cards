...
const linkStars = (repo) =>
  `<a href="${ghURL}/${repo.name}/stargazers" target="${repo.name}-stars">${starIcon} ${repo.stargazers_count}</a>`;
const linkForks = (repo) =>
  `<a href="${ghURL}/${repo.name}/forks" target="${repo.name}-forks">${forkIcon} ${repo.forks_count}</a>`;
  ...
  const h = sel.map((r) => {
    ...
    return `<div class="project-card">
      <h2 class="pcard-title flex-edge-cols">
        <span><a href="${ghURL}/${name}" target="${name}">${name}</a></span>${stars(
      r
    )}
      </h2>
      <div>
...
const total = () => `
<span class="stars">
  <span><a href="${ghURL}?tab=repositories" target="_blank">${repoIcon} ${reposCount}</a></span>
